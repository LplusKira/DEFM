...
sae_opt.rho = 0.1;
sae_opt.beta = 0.0;
sae_opt.b_s = ceil(size(X,1) / 50) * 2;			% use training and testing jointly
sae_opt.max_iter = 1;
sae_opt.max_cd = 5;
sae_opt.GAUSSIAN = 0;
    ...
    for a = 1 :1
        for b = 1 : 1
          ...
          %  crbm = CRBM_train(crbm,TRX,train_y,opt);
            ...
            print_opt(opt);
            [ sae,nn,crbm ] = NCEMSAEjointTrain( sae,nn,crbm,train_x,train_y,opt,test_x,test_y, sae_opt);
            
            [sae, test_code_x] = saeencode(sae, test_x);
