...
%% Do adjustment to fit rating-attribute project
function [w1_P1, w1_M1] = pmf(restart, num_feat, maxepoch, train_vec, probe_vec)
	...
	if restart==1 
		...
		numbatches = 50; % Number of batches
		num_m = max(max(train_vec(:,2)), max(probe_vec(:,2))); % Number of movies
		num_p = max(max(train_vec(:,1)), max(probe_vec(:,1))); % Number of users
		num_feat = num_feat; % Number of rank to decomposite
	...
	for epoch = 1:maxepoch
		...
		for batch = 1:numbatches
			...
			aa_p   = double(train_vec((batch-1)*N+1:min(n, batch*N),1));
			aa_m   = double(train_vec((batch-1)*N+1:min(n, batch*N),2));
			rating = double(train_vec((batch-1)*N+1:min(n, batch*N),3));
			...
			w1_M1_inc = momentum*w1_M1_inc + epsilon*dw1_M1;
			w1_M1 =  w1_M1 - w1_M1_inc;

			%w1_P1_inc = momentum*w1_P1_inc + epsilon*dw1_P1/N;
			w1_P1_inc = momentum*w1_P1_inc + epsilon*dw1_P1;
