...
function [w1_P1_sample, w1_M1_sample] = bayespmf(restart, num_feat, maxepoch, w1_P1, w1_M1, train_vec, probe_vec)
	...
	if restart==1 
		...
		%num_p = 6040;
		%num_feat = 10;
		num_m = max(max(train_vec(:,2)), max(probe_vec(:,2)));
		num_p = max(max(train_vec(:,1)), max(probe_vec(:,1)));
		num_feat = num_feat;
