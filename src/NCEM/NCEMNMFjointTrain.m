function [X, TX, DX, H, nn,crbm] = NCEMNMFjointTrain( nn,crbm,X,Y,DX,opt ,TX,TY, rating,H,UIdx,TUIdx,RUIdx,k,mf_opt)
...
%     end
    ...
    % Use NMF to update inputs
    %X(X < 0) = 0.0;						% set the negative values to zeros strictly !!!
    allX = [X; TX];
    W = allX(RUIdx, :);
    W = (W - ones(size(W,1), 1) * mean(W)) ./ (ones(size(W,1), 1) * std(W));			% standardize user weights
    W(isnan(W)) = 0.0;																	% remove nan
    W = W - ones(size(W,1), 1) * min(W);												% shift user weights
    fprintf('factorize sparse features by nnmf.\n');
    [W, H] = nnmf(rating, k, 'w0', W, 'h0', H, 'options', mf_opt);
    W = (W - ones(size(W,1), 1) * mean(W)) ./ (ones(size(W,1), 1) * std(W));			% standardize user weights
    W(isnan(W)) = 0.0;																	% remove nan
    W = W - ones(size(W,1), 1) * min(W);												% shift user weights
